...
const CAPPT_URL = process.env.CAPPT_URL || 'https://openapi.cappt.cc';
const CAPPT_TOKEN = process.env.CAPPT_TOKEN;

if (!CAPPT_TOKEN) {
    console.error('CAPPT_TOKEN environment variable is not set');
    process.exit(1);
}
...
server.tool(
    ...
    async ({outline, include_gallery}) => {
        const response = await fetch(`${CAPPT_URL}/ppt`, {
            method: 'POST',
            headers: {'Authorization': `Bearer ${CAPPT_TOKEN}`},
